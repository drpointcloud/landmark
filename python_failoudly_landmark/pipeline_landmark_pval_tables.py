...
path += datset + '_'
path += sys.argv[2] + hypothesis_test + '_' + '/'
...
if test_type == 'multiv':
    od_tests = []
    if hypothesis_test == "LMSW":
        md_tests = [MultidimensionalTest.LMSW.value]
    elif hypothesis_test == "MMD":
...
mmd_p_values = np.ones(( len(dr_techniques_plot),len(shifts))) * (-1)
            ...
            if sample == 1000:
                ...
                for dr_idx, dr in enumerate(dr_techniques_plot):
                    if hypothesis_test == "LMSW":
                        ...
                        mmd_p_values[dr_idx, shift_idx] = rand_run_p_vals[si,dr_idx,rand_run]
                    else:
                        pass
...
if hypothesis_test == "LMSW":
    ...
    if not os.path.exists(path2):
        os.makedirs(path2)
    np.savetxt("%s/%s_%s_%s_MMD_pval.csv" % (path2,datset,sign_level,shift_type), mmd_p_values, delimiter=",")
else:
    pass
