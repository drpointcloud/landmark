...
class ShiftLocator:
    ...
    def build_model(self, X_tr, y_tr, X_te, y_te, balanced=True):
        ...
        elif self.dc == DifferenceClassifier.LMSW:
            return self.landmark_class(X_tr, y_tr, X_te, y_te, balanced=balanced)
    ...
    def most_likely_shifted_samples(self, model, X_te_new, y_te_new):
        ...
        if self.dc == DifferenceClassifier.FLDA:
            ...
            return most_conf_test_indices, most_conf_test_perc, p_val < self.sign_level, p_val
                   #test_indices,          test_perc,           dec,                     p_val
            ...
            #y_te_new_pred = model.predict(X_te_new)
            y_te_new_pred = y_te_new[:,np.newaxis]

            # Get most anomalous indices sorted in descending order.
            most_conf_test_indices = np.argsort(y_te_new_pred)[::-1]
            most_conf_test_perc = np.sort(y_te_new_pred)[::-1]
            ...
            #shift_tester = ShiftTester(TestDimensionality.Multi)
            #landmark_test = ShiftTester.test_shift(self, X_tr, X_te)
            shift_tester = ShiftTester(TestDimensionality.Bin)
            p_val = shift_tester.test_shift_bin(successes, len(y_te_new_pred), self.ratio)
            #p_val = shift_tester.test_shift(successes, len(y_te_new_pred_argm), self.ratio)
    ...
    def landmark_class(self, X_tr, y_tr, X_te, y_te, balanced=False):
        # Partition the data set(s) for the difference classifier.
        X_tr_dcl, y_tr_dcl, y_tr_old, X_te_dcl, y_te_dcl, y_te_old = self.__prepare_difference_detector(X_tr, y_tr,X_te, y_te,balanced=balanced)
        #x_tr_new, y_tr_new, y_tr_old, x_te_new, y_te_new, y_te_old
        test_type = TestDimensionality.Multi.value
        md_test = MultidimensionalTest.LMSW.value
        sign_level = 0.1
        #shift_tester = ShiftTester(TestDimensionality(test_type), sign_level, mt=MultidimensionalTest(md_test))
        shift_tester = ShiftTester(TestDimensionality(test_type), sign_level=sign_level, mt=MultidimensionalTest(md_test))
        #landmark_test = shift_tester.test_shift(self, X_tr, X_te)
        p_val, _ ,landmark_divs,V = shift_tester.test_shift(X_tr_dcl, X_te)
        return V,landmark_divs,(X_tr_dcl, y_tr_dcl, y_tr_old, X_te_dcl, y_te_dcl, y_te_old )
            #model,score, ...
